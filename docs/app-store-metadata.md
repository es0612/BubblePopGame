# App Store Connect メタデータ

## 📱 アプリ基本情報

### アプリ名
- **日本語**: シャボン玉消しゲーム
- **英語**: Bubble Pop Game
- **文字数**: 日本語 9文字、英語 15文字（30文字制限内）

### サブタイトル
- **日本語**: 美しいシャボン玉で楽しむリラックスゲーム
- **英語**: Relaxing game with beautiful soap bubbles
- **文字数**: 日本語 19文字、英語 37文字（30文字制限内）

### カテゴリ
- **プライマリ**: ゲーム
- **セカンダリ**: アクション
- **ジャンル**: カジュアル, ファミリー

### 年齢制限
- **レーティング**: 4+（すべての年齢層に適用）
- **理由**: 暴力表現なし、不適切な内容なし

### 価格設定
- **価格**: 無料
- **アプリ内購入**: なし
- **広告**: なし

---

## 📝 説明文

### 日本語版説明文 (4000文字制限)

```
🫧 美しいシャボン玉を指先で楽しもう！

画面に舞い踊る虹色のシャボン玉をタップして消す、心癒されるカジュアルゲームです。美しいアニメーションと心地よい音楽で、日常の疲れを忘れてリラックスできます。

✨ 主な機能

【ゲームモード】
• 通常モード: 自由にシャボン玉を消してスコアを競う
• 数字順モード: 1から順番に数字のシャボン玉を消すチャレンジ
• 制限時間: 30秒〜180秒（初期値30秒）まで自由に設定可能
• シャボン玉数: 10個〜50個まで調整可能

【カスタマイズ設定】
• BGMセレクション: 3つの癒し系楽曲から選択
• 効果音調整: 音量レベル細かく設定
• バイブレーション: タッチフィードバックのON/OFF
• プログレッシブ難易度: 数字モードで段階的に難しくなる設定

【スコア・記録システム】
• ハイスコア自動保存
• ゲームモード別ランキング
• 制限時間別記録管理
• 詳細統計情報（正確率、プレイ時間など）

【充実のサポート機能】
• 分かりやすいチュートリアル（4ステップガイド）
• VoiceOver完全対応（視覚障害者サポート）
• Dynamic Type対応（文字サイズ調整）
• 色覚異常者配慮のカラーシステム

🎵 音響・体験の特徴
• 3曲のオリジナルBGM搭載
• リアルなシャボン玉破裂音
• 美しいパーティクル効果
• 滑らかな60FPSアニメーション
• 直感的なタッチ操作

📱 対応デバイス
• iPhone（iOS 17.0以降）
• iPad（iPadOS 17.0以降）
• レスポンシブデザインで全画面サイズ対応
• iPhone 12以降、iPad第9世代以降推奨

🌟 こんな方におすすめ
• 通勤・通学の隙間時間にリラックスしたい
• 集中力を鍛えたい（数字順モード）
• 美しい視覚効果を楽しみたい
• アクセシビリティ機能を必要とする
• シンプルで分かりやすいゲームをお探しの方

💫 アクセシビリティ
このアプリは誰もが楽しめるよう設計されています。VoiceOverによる音声ガイド、高コントラスト表示、文字サイズ調整など、障害をお持ちの方も安心してご利用いただけます。

🔒 プライバシー
ユーザーデータは全てデバイス内にローカル保存され、外部サーバーへの送信は一切行いません。安心してお楽しみください。

シンプルながらも奥深い、心温まるシャボン玉の世界をお楽しみください！
```

### 英語版説明文 (4000文字制限) — ⚠️ 絵文字禁止（ASC の en は絵文字を弾く。plain text + ハイフン箇条書き）

```
Pop beautiful soap bubbles with your fingertips!

A soothing casual game where you tap to pop rainbow-colored soap bubbles dancing on your screen. Beautiful animations and relaxing music help you forget daily stress and unwind.

Key Features

Game Modes
- Normal Mode: Freely pop bubbles and compete for high scores
- Numbered Mode: Challenge yourself by popping numbered bubbles in order
- Time Limit: Customizable from 30 to 180 seconds (default 30)
- Bubble Count: Adjustable from 10 to 50 bubbles

Customization Settings
- BGM Selection: Choose from 3 soothing music tracks
- Sound Effects: Fine-tune volume levels
- Vibration: Toggle touch feedback ON/OFF
- Progressive Difficulty: Gradually increasing challenge in numbered mode

Score and Record System
- Automatic high score saving
- Rankings by game mode
- Records management by time limit
- Detailed statistics

Comprehensive Support Features
- Easy-to-understand tutorial
- Full VoiceOver support (visual impairment accessibility)
- Dynamic Type support (text size adjustment)
- Color vision deficiency considerate color system

Audio and Experience
- 3 original BGM tracks included
- Realistic bubble popping sounds
- Beautiful particle effects
- Smooth 60FPS animations
- Intuitive touch controls

Device Compatibility
- iPhone (iOS 17.0 or later)
- iPad (iPadOS 17.0 or later)
- Responsive design for all screen sizes

Perfect For
- Relaxing during commute or study breaks
- Training concentration skills (numbered mode)
- Enjoying beautiful visual effects
- Users requiring accessibility features
- Anyone seeking simple, intuitive games

Accessibility
This app is designed for everyone to enjoy. With VoiceOver audio guidance, high contrast display, text size adjustment, and more, users with disabilities can use it with confidence.

Privacy
All data is stored locally on your device, with no transmission to external servers. Enjoy with peace of mind.

Experience the simple yet profound, heartwarming world of soap bubbles!
```

---

## 🔍 キーワード戦略

### 日本語キーワード (100文字制限)
```
シャボン玉,バブル,ゲーム,リラックス,カジュアル,癒し,アクション,パズル,家族,子供,大人,数字,順番,タイミング,反射神経,集中力,無料,シンプル,美しい,アニメーション,音楽,BGM,効果音,アクセシビリティ,VoiceOver
```

### 英語キーワード (100文字制限)
```
bubble,pop,soap bubbles,casual game,relaxing,family,kids,adults,numbers,sequence,timing,reflexes,concentration,free,simple,beautiful,animation,music,accessibility,VoiceOver,puzzle,action
```

---

## 🏷️ カテゴリ詳細

### プライマリカテゴリ: ゲーム
- **理由**: ユーザーがゲームとして認識し検索しやすい
- **競合**: 他のカジュアルゲームとの比較優位性

### セカンダリカテゴリ: アクション
- **理由**: タップ操作による即座の反応が求められる
- **ターゲット**: アクションゲーム好きのユーザー獲得

### 追加適合カテゴリ
- **ファミリー**: 全年齢対象の安全なコンテンツ
- **パズル**: 数字順モードの戦略性
- **エンターテインメント**: リラクゼーション要素

---

## 📱 年齢制限詳細

### レーティング: 4+
- **暴力表現**: なし
- **性的内容**: なし
- **薬物・アルコール**: なし
- **ギャンブル**: なし
- **不適切な言語**: なし
- **恐怖要素**: なし

### レーティング根拠
- 純粋な娯楽コンテンツ
- 教育的価値（集中力、反射神経向上）
- 家族全員で楽しめる内容

---

## 💰 価格戦略

### 無料配布の理由
1. **ユーザー獲得**: 初期ダウンロード数最大化
2. **口コミ効果**: 低い参入障壁による拡散期待
3. **アクセシビリティ**: 誰でも気軽に試せる
4. **レビュー獲得**: 評価とフィードバック収集

### 将来のマネタイズ検討
- **プレミアム版**: 追加BGM、スキン、モード
- **広告表示**: 非侵入的なバナー広告
- **アプリ内購入**: 装飾要素、追加機能

---

## 📸 スクリーンショット戦略

### iPhone用（必須: 6.7", 6.5", 5.5"）

#### Screenshot 1: メニュー画面
- **目的**: アプリの第一印象とブランディング
- **要素**: アプリタイトル、メインボタン、美しいUI
- **キャプション**: 「美しいシャボン玉の世界へようこそ」

#### Screenshot 2: 通常モードゲーム画面
- **目的**: 基本的なゲームプレイ紹介
- **要素**: カラフルなシャボン玉、スコア表示、タイマー
- **キャプション**: 「自由にシャボン玉を消してスコアを競おう」

#### Screenshot 3: 数字順モードゲーム画面
- **目的**: 特別なゲームモード訴求
- **要素**: 数字付きシャボン玉、チャレンジ要素
- **キャプション**: 「数字を順番に消すチャレンジモード」

#### Screenshot 4: ハイスコア画面
- **目的**: 競争要素とリプレイ価値アピール
- **要素**: スコアランキング、統計情報
- **キャプション**: 「あなたの記録を更新しよう」

#### Screenshot 5: 設定画面
- **目的**: カスタマイズ性とアクセシビリティ訴求
- **要素**: 音響設定、ゲーム設定、アクセシビリティ
- **キャプション**: 「あなた好みにカスタマイズ」

### iPad用（推奨: 12.9"）
- iPhone版と同様の内容
- より大画面での美しい表示を強調
- マルチタスキング対応の訴求

---

## 🎯 プロモーションテキスト

### 日本語 (170文字制限)
```
🫧 心癒されるシャボン玉ゲーム！美しいアニメーションと癒しの音楽で日常のストレスを忘れましょう。通常モードでリラックス、数字順モードでチャレンジ。VoiceOver対応で誰でも楽しめます。今すぐ無料でダウンロード！
```

### 英語 (170文字制限) — ⚠️ 絵文字禁止（ASC の en フィールドは絵文字を弾く）
```
Soothing bubble popping game! Beautiful animations and healing music help you forget daily stress. Relax in normal mode, challenge in numbered mode. VoiceOver support for everyone. Free download now!
```

---

## 🌍 地域別配布戦略

### Phase 1: 日本市場
- **理由**: 開発言語・文化的親和性
- **期間**: リリース〜3ヶ月
- **目標**: 1,000ダウンロード、4.0星評価

### Phase 2: 英語圏展開
- **対象**: 米国、英国、カナダ、オーストラリア
- **期間**: 4〜6ヶ月目
- **準備**: 英語版完全ローカライズ確認

### Phase 3: アジア太平洋
- **対象**: 韓国、台湾、香港、シンガポール
- **期間**: 7〜12ヶ月目
- **検討**: 追加言語対応

---

## 📊 競合分析

### 直接競合
1. **バブルシューター系**: より複雑、戦略性重視
2. **カジュアルタップゲーム**: シンプルさで類似
3. **リラクゼーションアプリ**: 癒し要素で競合

### 差別化ポイント
- **アクセシビリティ**: VoiceOver完全対応
- **カスタマイズ性**: 豊富な設定オプション
- **教育的価値**: 集中力・反射神経向上
- **プライバシー**: ローカル保存のみ

---

## 🎭 ブランドトーン

### 日本語版
- **親しみやすさ**: 「〜しましょう」「お楽しみください」
- **癒し系**: 「心癒される」「リラックス」
- **包容力**: 「誰でも」「安心して」

### 英語版
- **フレンドリー**: "Enjoy", "Experience", "Perfect for"
- **ポジティブ**: "Beautiful", "Soothing", "Wonderful"
- **インクルーシブ**: "Everyone", "All ages", "Accessible"

---

## 📈 Success Metrics

### ダウンロード数
- **週1**: 50件以上
- **月1**: 200件以上
- **月3**: 1,000件以上

### ユーザー評価
- **平均評価**: 4.0星以上維持
- **レビュー数**: 月10件以上
- **評価分布**: 4-5星が80%以上

### エンゲージメント
- **アプリ継続率**: 1日後50%以上
- **平均セッション**: 3分以上
- **月間アクティブ**: ダウンロード数の30%以上

---

*最終更新: 2026-05-30（v1.0 提出に向けて最新化）。反映済みの実装: 制限時間 30〜180秒（初期値30）/ iPhone・iPad 両対応 / リザルト画面はお祝いトーン（タイトル「おしまい！/All Done!」）/ 詳細統計はプレイ時間中心（平均反応速度・密度は廃止）/ 広告・解析・ネットワークなし＝データ収集なし。*
*⚠️ en（英語ロケール）の Description / Promotional / What's New は **絵文字禁止**（ASC が弾く）。ja は絵文字OK。提出直前に App Store Connect の最新ガイドラインと再確認すること。*